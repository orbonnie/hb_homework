...
from functions import total_meal_price
...
def join_strings(words):
    ...
    for word in words: long_string += word
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    for i in range(mid):
        items[i], items[last_ind - i] = items[last_ind - i], items[i]
...
def duplicates(items):
    ...
    for item in items:
        if item in unique:
            if item not in dups: dups.append(item)
