...
def top_chars(phrase):
    ...
    for x in phrase:
        if x in chars: chars[x] += 1
        else: chars[x] = 1

    desc_chars = sorted(chars.items(), key = lambda item: item[1], reverse = True)

    return [x[0] for x in desc_chars if x[1] == desc_chars[0][1]]
